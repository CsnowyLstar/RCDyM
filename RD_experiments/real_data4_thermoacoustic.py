...
def args(): 
    ...
    parser.add_argument('--spectral_radius', type=float, default=1.25)
...
###  (2) Read data                                           ###
################################################################
data = np.loadtxt('real_data/thermoacoustic.txt', delimiter='\t')[120000:,1]
X = data[:,None]
ts = np.linspace(0,8,X.shape[0])
dt = ts[-1]/(len(ts)-1)
tpoints = len(ts)
tp = 6.2
...
def monotonic_polyfit_nonnegative(x_data, y_data, degree, alpha=0.1):
    ...
    bounds = [(None, None)]
    bounds.extend([(-1, None) for _ in range(degree)])
