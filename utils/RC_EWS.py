...
class RC_EWM:
    def __init__(self, X, ts, window=2000, step=100, args=None, iscontinuous=True, isdetrend=False, ispde=False):
    ...
    def find_period_autocorr1(self, flo_x, Tmin=100, threshold=0.95):
        V = flo_x.shape[1]
        fflo_x = flo_x[::-1]
        T = Tmin
        ...
        selected_peaks = [peak for peak in peaks if corrs[peak] > threshold]
        if selected_peaks == []:
            period = Tmin
        elif selected_peaks[0] == Tmin:
    ...
    def get_max_floquet_con(self): 
        ...
        for j in tqdm(range(nwin)):
            ...
            period = self.find_period_autocorr1(flo_x)
    ...
    def get_max_lyapunov_con(self):
        ...
        for j in tqdm(range(nwin)):
            ...
            if isaut:
                ...
                rt = ri[args.warm_up+j*step:args.warm_up+j*step+pl,:,0]
            #num_lyaps = X.shape[1]
            num_lyaps = 1
    ...
    def get_max_floquet_dis(self, Tmin=100, threshold=0.95):
        ...
        for j in tqdm(range(nwin)):
            ...
            flo_dt = inte*dt
            period = self.find_period_autocorr1(flo_x, Tmin=Tmin, threshold=threshold)
    ...
    def get_max_lyapunov_dis(self):
        ...
        for j in tqdm(range(nwin)):
            ...
            if isaut:
                ...
                rt = ri[args.warm_up+j*step:args.warm_up+j*step+pl,:] 
